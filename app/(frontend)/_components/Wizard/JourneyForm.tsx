...
import { WizardStepType, WizardStep, WizardJourney } from '@/lib/types/wizard';
...
export function JourneyForm({
    ...
    if (label && !slugTouched) {
      ...
      if (generatedSlug && generatedSlug !== originalSlug) {
        startTransition(async () => {
          ...
          setSlugError(isUnique ? '' : `The slug '${generatedSlug}' is already in use. Please choose a different one.`);
        });
      } else {
        setSlugError('');
      }
  ...
  const checkSlugUniqueness = useCallback(async (value: string) => {
    ...
    startTransition(async () => {
      ...
      setSlugError(isUnique ? '' : `The slug '${value}' is already in use. Please choose a different one.`);
    });
  ...
  const addStep = () => {
    ...
    const backendStepType = stepType === WizardStepType.PredefinedComponent 
      ? WizardStepType.Predefined 
      : stepType;
    ...
    const newStep: WizardStep = {
      ...
      label: stepLabel || 
        (selectedTemplate ? selectedTemplate.title : 
         selectedPredefined ? selectedPredefined.name : `Step ${steps.length + 1}`),
      ref: stepType === WizardStepType.Template 
        ? (selectedTemplate?.id ?? '') 
  ...
  const handleSubmit = () => {
    ...
    startTransition(async () => {
      try {
        ...
        setToast({ message: `Journey ${isEdit ? 'updated' : 'created'} successfully!`, type: 'success', open: true });
      } catch (err) {
        console.error(`Failed to ${isEdit ? 'update' : 'create'} journey:`, err);
        setToast({ message: `Failed to ${isEdit ? 'update' : 'create'} journey`, type: 'error', open: true });
      }
    });
  };
  ...
  return (
    <div className="template-card p-6 lg:p-8 space-y-8 border">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-semibold mb-2">{isEdit ? 'Edit Journey' : 'Create New Wizard Journey'}</h1>
        {isEdit && onDelete && (
          <div className="flex items-center gap-2">
            <Link 
              href={`${process.env.NEXT_PUBLIC_BASE_PATH || '/xpm'}/wizard/${slug || journeyId}`}
            ...
            <button
              ...
              className="button-danger button-sm flex items-center gap-1"
            >
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          {slugError ? (
            ...
            <span className="text-xs text-gray-500">This will be used in the URL for direct access to this journey</span>
          )}
        </div>
      </div>

      {/* Step builder */}
      <div className="bg-muted rounded-lg p-4 border-t border-border space-y-6">
        ...
        <div>
          ...
          {steps.length === 0 ? (
            ...
            <div className="space-y-3 max-h-[400px] overflow-y-auto pr-1 pb-2">
              {steps.map((s, idx) => {
                const templateData = s.type === WizardStepType.Template ? getTemplateForStep(s) : null;
                return (
                  <div key={s.id} className="border border-border rounded-lg bg-background transition group overflow-hidden">
                    ...
                    <div className="flex items-center px-3 py-2 border-b border-border bg-muted/30 min-w-0">
                      ...
                      <span className="font-medium text-gray-800 mr-2 truncate min-w-0 flex-grow" title={s.label}>
                        {s.label}
                      </span>
                    ...
                    <div className="px-3 py-2 flex flex-col sm:flex-row gap-3 items-start">
                      <div className="flex-grow min-w-0">
                        <div className="flex items-center gap-1.5 mb-1">
                          ...
                          <span className="text-xs font-mono bg-muted px-1.5 py-0.5 rounded text-gray-600">{s.type}</span>
                        </div>
                        <div className="flex items-center gap-1.5">
                          <span className="text-xs font-medium text-gray-500">Reference:</span>
                          <span className="text-xs text-gray-600 truncate max-w-[250px]" title={s.ref}>{s.ref}</span>
                        </div>
                      </div>
                      
                      {/* Template preview */}
                      {s.type === WizardStepType.Template && templateData && templateData.htmlContent && (
      ...
      <div className="flex gap-3">
        <button
          type="button"
          onClick={onCancel}
          className="button-secondary button-lg flex-1"
        >
          Cancel
        </button>
        <button
